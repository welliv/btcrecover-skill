...
├── CHECKSUMS.sha256              ← Signed file integrity manifest (auto-generated on release)
...
Note on `CHECKSUMS.sha256`: this file is automatically generated by
`.github/workflows/release.yml` when a version tag is pushed. It is
not hand-edited. A placeholder version is maintained in the repo between
releases. Always use the signed version from the GitHub Releases page.
...
```
Keybase identity
  └── proves GitHub account, X account, domain simultaneously
        └── GPG commit signing
              └── GitHub "Verified" badge on every commit
                    └── Cosign signing via GitHub Actions (every release)
                          └── Rekor transparency log (tamper-resistant)
                                └── SHA256 CHECKSUMS.sha256 (all files)
                                      └── install.sh verifies all three
```
...
- Unique visual fingerprint in SKILL.md header with current CHECKSUMS hash
...
fingerprint, Sigstore identity, and — critically — what the real skill never
...
authenticity chain (Keybase + GPG + Cosign) is established.
...
7. Fill in all welliv and [your address] placeholders
   ...
   - VERIFIED.md
   - README.md
   - AUTHENTICITY-AND-LIABILITY.md
   - install.sh
   - scripts/benchmark-updater.py
   - agents/hermes.md

8. Tag v1.0
   git tag -s v1.0 -m "Initial release"
   git push origin v1.0
   (triggers the release signing workflow)

9. Verify the signed artifacts appear in GitHub Releases
   Download CHECKSUMS.sha256.bundle
   cosign verify-blob --bundle CHECKSUMS.sha256.bundle [...]
...
12. First public announcement
    Post the canonical GitHub URL and Keybase URL together
    These two URLs must appear together in the first public post
    so they are permanently linked in search results
```

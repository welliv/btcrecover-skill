...
When asked to audit the build, run `scripts/verify-btcrecover.sh`, check for stale backup files (delete any `.bak*`), confirm hybrid recovery patterns are in `references/verified-scenarios.md`, and produce a brief substantive summary. No redundancy. Use simplified British English with natural tone. Respond to "remember our btcrecover session" by immediately recalling hybrid details (passphrase truncation, Sparrow BIP84 change address at index 10, addr-limit 100, seedrecover-cli with --passphrase-list).
